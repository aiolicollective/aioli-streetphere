...
::  [3] Flat satellite image (satmap, needs setup.bat)
::  [4] All three from the same URL (both.py, needs setup.bat)
:: ============================================================
...
echo   [3] Flat satellite image, true to scale (satmap)
echo   [4] All three from the same URL
echo   [Q] Quit
echo.
set CHOICE=
set /p CHOICE="  Choice > "
...
if /i "%CHOICE%"=="3" goto :satmap
if /i "%CHOICE%"=="4" goto :both
...
if errorlevel 1 (
    ...
    echo.
    echo   Options [1] and [3], the 360 sphere and the satellite image,
    echo   work without either of them.
    echo.
)
goto :menu
...
if not exist venv\Scripts\python.exe (
    echo.
    echo  [!] The all-three mode needs the venv: run setup.bat first.
    goto :menu
)
call venv\Scripts\activate.bat
python both.py
